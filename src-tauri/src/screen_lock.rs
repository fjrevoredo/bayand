...
mod imp {
    ...
    fn trigger_auto_lock(reason: &str) {
        if let Some(app) = APP_HANDLE.get() {
            let state = app.state::<crate::commands::auth::BayandState>();
            match crate::commands::auth::auto_lock_tracker_if_unlocked(state, app.clone(), reason) {
                Ok(true) => info!("Tracker auto-locked due to {}", reason),
                Ok(false) => {}
                Err(error) => warn!("Failed to auto-lock tracker ({}): {}", reason, error),
            }
        }
    }
