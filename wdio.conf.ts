...
const testAppDir = isCleanMode ? mkdtempSync(join(tmpdir(), 'bayand-e2e-app-')) : null;
const testWebviewDir =
  isCleanMode && process.platform === 'win32'
    ? mkdtempSync(join(tmpdir(), 'bayand-e2e-webview-'))
    : null;
...
const appDataDir = isCleanMode ? testAppDir : statefulAppDir;
...
const appBinary = resolve(
  ...
  process.platform === 'win32'
    ? 'src-tauri/target/release/bayand.exe'
    : 'src-tauri/target/release/bayand',
);
  ...
  onPrepare: async () => {
    ...
    tauriDriver = spawn('tauri-driver', nativeDriverArgs(), {
      ...
      env: {
        ...
        ...(appDataDir
          ? {
              BAYAND_APP_DIR: appDataDir,
            }
          : {}),
        BAYAND_E2E: isCleanMode ? '1' : '0',
      },
    });
